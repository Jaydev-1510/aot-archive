...
export function buildRelationshipStatements(relationships: SeedRelationship[]): string[] {
  ...
  for (const r of relationships) {
    statements.push(
      ...buildUpdateThenInsert(
        "relationships",
        [
          "subject_id", "subject_type", "predicate", "object_id", "object_type", "qualifier",
          "year_start", "year_end", "date_precision", "canon_status", "source_id", "notes",
        ],
        [
          r.subject, raw(entityTypeSubquery(r.subject)), r.predicate, r.object, raw(entityTypeSubquery(r.object)),
          r.qualifier ?? null,
          r.yearStart, r.yearEnd, r.datePrecision, r.provenance?.canonStatus ?? DEFAULT_CANON_STATUS,
          r.provenance?.source ? raw(sourceIdSubquery(r.provenance.source)) : null,
        ...
        ["subject_type", "object_type", "year_start", "year_end", "date_precision", "canon_status", "source_id", "notes"],
      ),
    );
  }
  return statements;
